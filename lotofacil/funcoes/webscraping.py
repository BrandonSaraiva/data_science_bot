...
def verificar_tecla_p():
    while True:
        if keyboard.is_pressed('p'):
            print("Usuário pressionou a tecla 'p'. Encerrando a execução...")
            os._exit(0)
        time.sleep(0.1)
def scraping():
    ...
    # Iniciar a verificação da tecla 'p' em uma thread separada
    p_thread = threading.Thread(target=verificar_tecla_p)
    p_thread.start()
